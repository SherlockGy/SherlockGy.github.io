...
export function normalizeManifest(data, base) {
  ...
  return data.albums.map(album => {
    ...
    if (!validDate(album.date)) throw new Error('图集日期须为有效的 YYYY-MM-DD');
    if (!Array.isArray(album.images) || !album.images.length) throw new Error('每个图集至少需要一张图片');
    return {
      id: album.id, title: album.title.trim(), date: album.date,
...
export function filterAlbums(albums, { query = '', month = '', type = 'all' } = {}) {
  const terms = query.trim().toLocaleLowerCase().split(/\s+/).filter(Boolean);
  return albums.filter(album => (!month || album.date.startsWith(month)) &&
