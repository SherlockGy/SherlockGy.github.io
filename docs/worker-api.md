...
单独修改名称或说明时仍提交原图片顺序，不传图片文件；也可与排序、新图一次保存。修改名称或说明保持图集 ID、原图地址、日期、系列、标签及分享链接。自动生成的图片说明会随名称和当前图片顺序更新，自定义图片说明保留。名称和图集说明变化均参与版本冲突与请求指纹校验；未携带新增字段的旧请求保留原指纹格式，兼容旧草稿重试。说明为空与未提交说明有不同含义，Worker 用字段是否存在来区分。
