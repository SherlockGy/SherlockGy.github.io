...
首页只需要图集封面。构建脚本读取每个图集当前第一张图片，生成 480 / 960 两档 WebP；原图继续供逐页阅读、连续阅读和下载。保留完整构图与透明度，小图不放大，动画取第一帧作为静态封面，EXIF 方向在缩略图中应用。有有效 ICC 颜色配置时先转为 sRGB，避免直接丢弃配置造成偏色。
...
- 只生成每个图集当前第一张图片的封面。编辑排序预览复用已发布的同一组缩略图，按原图 URL 匹配，避免重排或换图后串图；没有对应缩略图的图片按需加载原图。阅读器侧栏的其他小图保持原有行为。
