...
- 同一页面原样重试会复用已经成功的图片凭据。凭据有效期 24 小时，签名快照有效期 5 分钟；过期快照在服务端重新读取。图片凭据被服务端判为过期或无效时，前端清除这些凭据，下次原样重试重新上传，当前文件与排序继续保留。
...
2. 流水上传从 `2026-09-17-upload-pipeline-1` 开始支持；当前 `2026-09-17-review-3` 继续返回同一 `upload.protocol`，并支持名称与说明编辑。
...
6. 轮换 GitHub Token 或上传口令会使旧签名凭据失效。前端收到凭据无效错误后会清除旧凭据，下次原样重试重新上传，无需重新选择图片；若上传口令变更，先填写新口令。
