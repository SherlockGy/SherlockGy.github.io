...
写入仍由 Cloudflare Worker 处理。当前版本为 `2026-09-17-review-3`：支持名称与说明编辑，并在排序保存时按实际顺序更新自动生成的页码说明，保留自定义说明和原图地址。旧目录中已错位的自动页码由前端在显示时纠正，不改源文件。缩略图、弹窗布局和缩放比例展示不依赖 Worker 升级；Secret 保持原值。更新仓库内的 Worker 文件不会自动部署 Cloudflare，详见 [部署说明](worker/SETUP.md)。不要把 GitHub 凭据放在前端；这是公开的个人图集站，发布的图片可被访问。
