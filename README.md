...
- 已有图集支持修改名称、新增图片、排序和换图；一次保存全部生效。仅上传新增或替换的文件，单独改名不上传图片。
...
5. 阅读已有图集时点击“编辑图集”，载入最新内容后可以修改名称、新增、排序、换图。可以移除本次尚未保存的新图，或撤销本次换图。名称为 1–120 字，改名保持图集 ID、图片地址和分享链接。
...
写入仍由 Cloudflare Worker 处理。缩略图功能不依赖 Worker 升级；本次新增改名功能需要部署 `2026-09-17-album-titles-1` 版 Worker，Secret 保持原值。推荐先部署新版 Worker，再发布前端；旧服务下名称为只读，原有图片编辑仍可使用。更新仓库内的 Worker 文件不会自动部署 Cloudflare，详见 [部署说明](worker/SETUP.md)。不要把 GitHub 凭据放在前端；这是公开的个人图集站，发布的图片可被访问。
