...
- 发布时为现有和新增图集的每张本地图片生成 480 / 960 像素 WebP 缩略图，首页、编辑排序和放映目录按屏幕选图；阅读与下载保持使用原图。
- 支持搜索名称、说明和标签，筛选单张或多图。
- 支持逐页阅读、连续阅读、放大、适屏、查看原图和复制当前页链接。两种阅读模式默认按阅读框的宽度和高度等比例显示整张图片；连续模式滚动切换图片，手动放大后可滚动查看细节。
...
- 添加、编辑和系列管理弹窗中，顶部关闭按钮与底部保存按钮始终可见，中间内容独立滚动；上传进度、保存结果与错误提示固定在底栏，长提示可独立滚动阅读。
...
5. 阅读已有图集时点击“编辑图集”，载入最新内容后可以修改名称与说明、新增、排序、换图。可以移除本次尚未保存的新图，或撤销本次换图。名称为 1–120 字，说明最多 1000 字、允许清空；修改名称或说明保持图集 ID、图片地址和分享链接。
...
写入仍由 Cloudflare Worker 处理。缩略图和弹窗布局不依赖 Worker 升级；名称与说明编辑由 `2026-09-17-album-details-1` 版 Worker 完整支持，Secret 保持原值。推荐先部署新版 Worker，再发布前端；旧服务未支持的字段为只读，原有图片编辑仍可使用。更新仓库内的 Worker 文件不会自动部署 Cloudflare，详见 [部署说明](worker/SETUP.md)。不要把 GitHub 凭据放在前端；这是公开的个人图集站，发布的图片可被访问。
